...
# Autoround config classes need specific import
...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen3 Online Quantization Script")
    parser.add_argument('--model_path', type=str, default="/workspace/weights/Qwen3-30B",
                        help="Path to the float model")
    parser.add_argument('--save_path', type=str, default="/workspace/weights/Qwen3-30B-W4A4-OnlineQuaRot",
...
if __name__ == "__main__":
    ...
    if args.calib_file:
        ...
        # Structure: msit/msmodelslim/qwen3_quantization/quarot_online/script.py
    ...
    quarot_config = QuaRotProcessorConfig(
        online=True,  # Enable Online Rotation
        block_size=-1,
        max_tp_size=4,
        down_proj_online_layers=[] # No specific layers for down_proj online rotation by default
    )
    ...
    print("Online Quantization Finished!")
